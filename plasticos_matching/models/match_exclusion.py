...
class PlasticosMatchExclusion(models.Model):
    ...
    def _cron_expire_temporary_exclusions(self):
        ...
        """
        self.env.cr.execute(
            "SELECT pg_try_advisory_lock(hashtext(%s))", ["plasticos_matching.ir_cron_expire_exclusions"]
        )
        ...
        try:
            ...
            self.env.cr.execute(
                "SELECT pg_advisory_unlock(hashtext(%s))", ["plasticos_matching.ir_cron_expire_exclusions"]
            )
