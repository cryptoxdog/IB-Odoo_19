"""Graph outbox consumer.

Processes pending events from the outbox queue and syncs them to Neo4j.
"""
...
class GraphOutboxConsumer(models.AbstractModel):
    ...
    _description = "Graph Outbox Consumer"

    @api.model
    def process_queue(self):
        """Process pending outbox events and sync to Neo4j."""
        ...
        for event in outbox:
            try:
                ...
                if handler:
                    ...
                    _logger.warning("No handler for model %s", event.model_name)
                event.mark_processed()
            except Exception as e:
                _logger.error("Outbox event %s failed: %s", event.id, e)
