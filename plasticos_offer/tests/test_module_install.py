"""
Skeleton smoke tests for plasticos_offer.
...
class TestModuleInstall(PlasticosTestCase):
    """Verify plasticos_offer installs cleanly and registers its models."""

    def test_module_installed(self):
        """Module plasticos_offer should be in installed state."""
        module = self.env["ir.module.module"].search(
            [
                ("name", "=", "plasticos_offer"),
            ],
            limit=1,
        )
        self.assertTrue(module, "Module plasticos_offer not found in registry")
        self.assertEqual(module.state, "installed", f"Expected installed, got {module.state}")

    def test_models_registered(self):
        """All plasticos_offer models must be in the registry."""
        expected_models = [
            "plasticos.offer",
        ]
    ...
    def test_inherited_models_accessible(self):
        """Inherited models must remain accessible after plasticos_offer extension."""
        inherited_models = [
            "res.partner",
            "mail.thread",
        ]
