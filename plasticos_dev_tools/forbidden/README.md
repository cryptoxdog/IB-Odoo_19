...
|:-------|:----------|:-------|
| `buyer_matching_runtime_v6.0C.py` | Probabilistic scoring / AI matching | Computes match scores between offers and buyers using weighted algorithms. References non-existent `plasticos.offer` and `plasticos.buyer.profile` models. |
| `trust_index_calculator_v6.0C.py` | Probabilistic scoring | Calculates composite trust scores using weighted formulas. References non-existent `plasticos.buyer.profile` model. |
| `offer_handler.py` | Transaction state mutation | Creates and dispatches offers, modifies sale orders. Direct accounting-adjacent mutation. |
...
| `plastos_event_logger_v6.0.py` | References non-existent models | Depends on `plasticos.decision_ledger` which does not exist in the repo. |
