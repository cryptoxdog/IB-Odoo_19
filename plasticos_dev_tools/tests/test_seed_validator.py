...
class TestSeedValidator(PlasticosTestCase):
    ...
    def test_document_tag_uniqueness(self):
        ...
        for tag in tags:
            if tag.code in seen_codes:
                duplicates.append(f"Code '{tag.code}': IDs {seen_codes[tag.code]} and {tag.id}")
            else:
                ...
                if code.startswith("model."):
                    ...
                    if model_name and model_name in self.env:
                        ...
                        if not hasattr(model_obj, method_name):
                            missing_methods.append(f"Cron '{cron.name}': method '{method_name}' not on '{model_name}'")

        self.assertEqual(len(missing_methods), 0, f"Found crons with missing methods: {missing_methods}")
    ...
    def test_acl_model_references(self):
        ...
        self.assertEqual(len(invalid), 0, f"Found ACLs with invalid models: {invalid}")
