...
class TestModelRegistration:
    ...
    def test_sql_constraints_have_unique_names(self):
        """_sql_constraints entries must have unique constraint names per module.

        Note: _sql_constraints IS valid in Odoo 19 for database-level unique
        constraints. This test validates that constraint names don't collide.
        """
        constraint_names = []
        for fname, src in _collect_python_files("models"):
            # Extract constraint names from _sql_constraints = [('name', ...)]
            matches = re.findall(r"_sql_constraints\s*=\s*\[(.*?)\]", src, re.DOTALL)
            for match in matches:
                names = re.findall(r"\(\s*['\"](\w+)['\"]", match)
                for name in names:
        ...
        for fname, name in constraint_names:
            if name in seen:
                pytest.fail(f"Duplicate SQL constraint name '{name}' in {fname} (also in {seen[name]})")
            seen[name] = fname
...
class TestNeo4jOntologyAlignment:
    ...
    def test_transaction_edge_uses_merge(self):
        ...
        # Check for MERGE with SOLD_TO relationship (whitespace-tolerant)
        assert "SOLD_TO" in self.src and "MERGE" in self.src
...
class TestCypherDeterminism:
    ...
    def test_relaxed_query_only_polymer_hard(self):
        ...
        assert (
            ...
            or "// ONLY HARD GATE:" in self.graph_src
        )
    ...
    def test_gate_mode_strict_flexible_optimistic(self):
        ...
        for mode in ["strict", "flexible", "optimistic"]:
            # Check for gate_mode with or without object prefix (buyer.gate_mode or gate_mode)
            pattern = f"gate_mode = '{mode}'" in self.graph_src or f".gate_mode = '{mode}'" in self.graph_src
            assert pattern, f"gate_mode '{mode}' not found in calculate_match_score"
