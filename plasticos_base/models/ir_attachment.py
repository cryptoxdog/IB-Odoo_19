...
class IrAttachment(models.Model):
    ...
    def _cron_cleanup_missing_filestore_orphans(self):
        ...
        self.env.cr.execute(
            "SELECT pg_try_advisory_lock(hashtext(%s))", ["plasticos_base.ir_cron_cleanup_attachment_orphans"]
        )
        ...
        try:
            ...
            self.env.cr.execute(
                "SELECT pg_advisory_unlock(hashtext(%s))", ["plasticos_base.ir_cron_cleanup_attachment_orphans"]
            )
